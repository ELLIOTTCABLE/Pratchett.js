...
util.constructify = (opts)->
   inner = (body)->
      ...
      # (understandably). This means that our `Wrapper` is no longer the *actual type* we're trying
      # to construct (that is, the nominal “constructor function” whose prototype we're trying to
      # construct.)
      #
      # My approach to solving this is to replace *our wrapper*'s prototype with CoffeeScript's-
      # wrapper's prototype, the first time it *looks* like we're getting called from a CoffeeScript
      # wrapper.
      #
      ...
      #
      # This *SHOULD NOT* affect you if you're using this in the most common, intended fashion (as a
      # direct tag on a CoffeeScript class.)
      #
      ...
      before_interceptor = ->
         ...
         # `#apply`, which we've here intercepted. Second, we know that the wrapper itself will have
         # a defined (and non-zero-length) function-name (which *no other function* in CoffeeScript
         # can have); third, we know that the second argument given to `#apply` will be an
         # `arguments` object, and thus have a `callee` property, referring to the CoffeeScript
         # wrapper itself, our caller.
...
#
#  - First off, CoffeeScript *does not* generate constructors that explicitly return the constructed
#    value; it's assumed (reasonably enough) that the constructor will always be called via the
#    `new` invocaton pattern, which ensures such a return. When using a “parameterized call pattern”
#    as per this function's intended use, the final constructor itself is *not* called via a `new`
#    invocation pattern (even though such is simulated reasonably well.)
#
...
#                 if (@_.beAwesome) ...
#
#    The idiomatic way around this, is to store the options object to a local variable if you will
